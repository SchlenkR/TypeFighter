...
module Env =
    ...
    let bind (exp: IExp) (env: Env) : Env =
        env |> Map.change exp.exp (fun _ -> Some(Intern exp.meta.tyvar))
...
module Annotation =
    ...
    let annotate (env: Env) (exp: UExp) =
        let newTyVar = newUpCounter(-1)
        let mutable allExpressions = List.empty<AstExp>
        let addExp exp = allExpressions <- exp :: allExpressions
...
module NewStuff =
    ...
    type Node = { id: NodeId; data: NodeData; desc: string }
    ...
    let createGraph (annoRes: AnnotationResult) =
        ...
        let rec constrain (texp: TExp) =
            [
                let thisTyvar = texp.meta.tyvar
                match texp.exp with
                | Lit x ->
                    let typeName = Lit.getTypeName x
                    let nsource = newNode typeName (Source (TApp (typeName, [])))
                    yield edge $"Lit({typeName})" nsource.id thisTyvar
                    yield node nsource
                | Var ident ->
                    let envItem = Env.resolve ident texp.meta.env
                    let envTyvar,externNode =
                        match envItem with
                        | Intern tyvar -> 
                            tyvar, None
                        | Extern tau ->
                            let n = newNode $"Extern {Format.tau tau}" (Source tau)
                            n.id, Some n
                    match externNode with Some n -> yield node n | _ -> ()
                    yield edge $"Var ({ident})" envTyvar thisTyvar
                | App (e1, e2) ->
                    let nfun = newNode "App" (MakeFun { inc1 = e2.meta.tyvar; inc2 = thisTyvar })
                    yield node nfun
                    yield edge "App (e1 = e2)" nfun.id e1.meta.tyvar
                    yield! constrain e2
                    yield! constrain e1
                | Abs (ident, body) ->
                    let nfun = newNode "Abs" (MakeFun { inc1 = ident.meta.tyvar; inc2 = body.meta.tyvar })
                    yield node nfun
                    yield edge $"Abs ({ident.exp})" nfun.id thisTyvar
                    yield! constrain body
                | Let (ident, e, body) ->
                    yield edge $"Let ({ident})" body.meta.tyvar thisTyvar
                    yield! constrain e
                    yield! constrain body
                | Prop (ident, e) ->
                    failwith "TODO: Prop Constraint"
                | Tuple es -> 
                    failwith "TODO: Tuple Constraint"
                | Record fields -> 
                    failwith "TODO: Record Constraint"
            ]
        let constraintRes = constrain annoRes.root
        let nonAstNodes = constraintRes |> List.collect snd
        let astNodes =
            ...
            [ for x in annoRes.allExpressions do
                let tyvar =
                    match x with
                    | EnvExp exp -> exp.meta.tyvar
                    | SynExp exp -> exp.meta.tyvar
