...
// Shared with the playground — single source of truth in `assets/logo.svg`.
// We inject `class="logo"` here so the docs CSS can size it; the playground
// scopes its own styles via a wrapper element.
let logoSvg =
    let raw = File.ReadAllText(Path.Combine(repoRoot, "assets", "logo.svg"))
    raw.Replace("<svg ", """<svg class="logo" """)
